# choose your compiler
CC=gcc
#CC=gcc -Wall

shell: sh.o get_path.o main.c
	$(CC) -pthread -g main.c sh.o get_path.o -o shell
#	./shell
#	$(CC) -g main.c sh.o get_path.o bash_getcwd.o -o mysh

pathmain: get_path.o get_path_main.o
	$(CC) -g get_path.o get_path_main.o -o pathmain

sh.o: sh.c sh.h alias.c base_commands.c history.c prompt.c setenv.c watchmail.c watchuser.c where.c which.c
	$(CC) -pthread -g -c sh.c

get_path.o: get_path.c get_path.h
	$(CC) -pthread -g -c get_path.c

get_path_main.o: get_path_main.c get_path.h
	$(CC) -pthread -g -c get_path_main.c

clean:
	rm -rf sh.o get_path.o shell ~#  *~ shell.dSYM *.o pathmain
